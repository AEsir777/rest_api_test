...
class ProductCreateTestCase(APITestCase):
    def test_create_product(self):
        ...
        product_attrs = {
            'name': 'New Product',
            'description': 'This is the descrption for the product',
            'price': '43432.43',
        }
        response = self.client.post('http://127.0.0.1:8000/api/example1/new', product_attrs)
...
class ProductDestoryTestCase(APITestCase):
    def test_delete_product(self):
        ...
        self.client.delete('http://127.0.0.1:8000/api/example1/{}/rud'.format(product_id))
        self.assertEqual(
            Product.objects.count(),
            initial_product_count - 1
        )
...
class ProductListTestCase(APITestCase):
    def test_list_products(self):
        ...
        response = self.client.get('http://127.0.0.1:8000/api/example1')
        # all the paginatin exist
...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        reponse = self.client.patch(
            'http://127.0.0.1:8000/api/example1/{}/rud'.format(product.id),
            {
                'name': 'New Product',
                'description': 'This is the descrption for the product',
                'price': '43432.43',
    ...
    def test_upload_product_photo(self):
        ...
        try:
            ...
            self.assertTrue(
                updated.photo.startswith(expected_photo)
            )
        finally:
